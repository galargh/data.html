...
(defn tokenize
  ...
  (case state
      ...
      (condp rich-compare next-input-character
        #"[A-Za-z]" (tokenize all-input-characters
                              (assoc tokenizer-state
                                ...
                                :token {:type :end-tag, :data ""}))
        (concat (list {:type :character, :data \<} {:type :character, :data \/})
                (tokenize all-input-characters
    ...
    :markup-declaration-open
      (cond (= (apply str (safe-peek all-input-characters 2)) "--")
    ...
    :comment (condp rich-compare next-input-character
               \< (tokenize remaining-input-characters
                            (assoc tokenizer-state
      ...
      (condp rich-compare next-input-character
        ...
        (tokenize all-input-characters
                  (assoc tokenizer-state
                    :state :comment
                    :token (assoc token :data (str data \-)))))
    :comment-end (condp rich-compare next-input-character
                   \> (cons token
                            (tokenize remaining-input-characters
                                      (assoc tokenizer-state
                                        :state :data
    ...
    :DOCTYPE
      (condp rich-compare next-input-character
        ...
        (tokenize remaining-input-characters
                  (assoc tokenizer-state
                    :state :DOCTYPE-name
                    :token {:type :DOCTYPE, :name (str next-input-character)})))
    :DOCTYPE-name
      (condp rich-compare next-input-character
        #{\tab \u000A \u000C \space} (tokenize remaining-input-characters
                                               (assoc tokenizer-state
